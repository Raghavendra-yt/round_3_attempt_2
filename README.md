...
Built with a **Python / FastAPI** backend and a **React + TypeScript** frontend, using **Google Gemini (Vertex AI)** for personalized advice and **Firestore** for tracking, designed with a premium eco-tech dark theme, and fully verified with **100% test coverage** on both backend and frontend.
...
- **Premium Eco-Tech Design**: A beautiful, dark slate-to-obsidian theme featuring frosted glassmorphic cards, typography from Google Fonts (**Outfit**), neon glowing accent states, custom scrollbars, and smooth `@keyframes` chart animations.
...
```bash
git clone https://github.com/Raghavendra-yt/Prompt_wars_3_attempt_1.git
cd Prompt_wars_3_attempt_1
```
